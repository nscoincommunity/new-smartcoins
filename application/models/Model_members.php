...
class Model_members extends CI_model{
    ...
    function kode_konsumenLama(){
        return $this->db->query("SELECT * FROM rb_konsumen a where a.status = 'free' ORDER BY a.id_konsumen DESC");
    }
    function username($kode){
        return $this->db->query("SELECT username FROM rb_konsumen  where kode_konsumen = $kode ");
    }
    ...
    function kode_konsumen_tambah(){
        for ($i = 1; $i <= $this->input->post('kode'); $i++){
            ...
            $noUrut = $row['id_konsumen']+1;

            //$noUrut++ ;
            $char = "CTR-ID-0.";
            $username = $char . sprintf("%08s", $noUrut);
    ...
    function list_member_baru() {
        return $this->db->query("SELECT * FROM rb_konsumen WHERE status='sold' ORDER BY id_konsumen DESC LIMIT 50");
    }

    function hapus_member($id) {
      $this->db->where('id_konsumen', $id);
      $this->db->delete('rb_konsumen');
