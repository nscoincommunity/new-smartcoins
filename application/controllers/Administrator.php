...
class Administrator extends CI_Controller {
	...
	function sw_setting_bonus(){
		cek_session_akses('sw_setting_bonus',$this->session->id_session);
		if (isset($_POST['submit'])){
			$this->model_investasi->setting_bonus_update();
			redirect('administrator/sw_setting_bonus');
		}else{
			$data['record'] = $this->model_investasi->get_setting_bonus();
			$this->template->load('administrator/template','administrator/mod_identitas/view_setting_bonus',$data);
		}
	}
	...
	function tambah_tag_tes() {
		$this->model_berita->tag_berita_tambah_test();
	}
	...
	function delete_konsumen(){
		cek_session_akses('konsumen',$this->session->id_session);
		$id = $this->uri->segment(3);
		$this->model_members->hapus_member($id);
		redirect('administrator/koderahasia');
	}

	function delete_konsumenaktif(){
		cek_session_akses('konsumen',$this->session->id_session);
		$id = $this->uri->segment(3);
		$this->model_members->hapus_member($id);
		redirect('administrator/konsumen');
	...
	function aktifkan_member(){
		cek_session_akses('konsumen',$this->session->id_session);
		$id = $this->uri->segment(3);
		$this->model_members->aktivasi_member($id);
		redirect('administrator/koderahasia');
	}
	...
	*/
	...
	function voucher_code(){
		cek_session_akses('voucher_code',$this->session->id_session);
		if (isset($_POST['submit'])){
			$kode = $this->input->post('kode');
			$this->model_investasi->kode_voucher_tambah();
			redirect('administrator/voucher_code');
		}else{
			$data['record'] = $this->model_investasi->voucher_new();
			$this->template->load('administrator/template','administrator/mod_investasi/voucher_code',$data);
		}
	}

	function voucher_terjual() {
		cek_session_akses('voucher_code',$this->session->id_session);
		$id = $this->uri->segment(3);
		$this->model_investasi->voucher_terjual($id);

		redirect('administrator/voucher_code');
	}

	function voucher_sold() {
		$data['record'] = $this->model_investasi->list_voucher_sold();
			$this->template->load('administrator/template','administrator/mod_investasi/voucher_terjual',$data);
	...
	function investasi(){
		cek_session_akses('investasi',$this->session->id_session);
			...
			$data['record'] = $this->model_investasi->list_investasi();
			$this->template->load('administrator/template','administrator/mod_investasi/view_inv',$data);

	}

	function investasi_aktif(){
		cek_session_akses('investasi',$this->session->id_session);

			$data['record'] = $this->model_investasi->list_investasi_aktif();
			//$data['total']	= $this->model_investasi->total_investasi_aktif();
			$this->template->load('administrator/template','administrator/mod_investasi/view_inv_aktif',$data);

	}

	function investasi_berakhir(){
		cek_session_akses('investasi',$this->session->id_session);

			$data['record'] = $this->model_investasi->list_investasi_berakhir();
			//$data['total']	= $this->model_investasi->total_investasi_aktif();
			$this->template->load('administrator/template','administrator/mod_investasi/view_inv_berakhir',$data);

	}


	function profit_harian() {
		cek_session_akses('investasi',$this->session->id_session);
			$data['record'] = $this->model_investasi->list_profit_harian();
			$this->template->load('administrator/template','administrator/mod_investasi/view_profit_harian',$data);

	}


	function permintaan_wd() {
		cek_session_akses('investasi',$this->session->id_session);
			$data['record'] = $this->model_investasi->list_wd_baru();
			$this->template->load('administrator/template','administrator/mod_investasi/view_wd_baru',$data);

	}

	function wd_terkirim() {
		cek_session_akses('investasi',$this->session->id_session);
			$data['record'] = $this->model_investasi->list_wd_terkirim();
			$this->template->load('administrator/template','administrator/mod_investasi/view_wd_terkirim',$data);

	}
	
	function ccm_running() {
	    cek_session_akses('investasi',$this->session->id_session);
	        
	    	$data['record'] = $this->model_investasi->list_investasi_aktif();
			$this->template->load('administrator/template','administrator/mod_investasi/view_ccm_running',$data);
	}

	function penarikan_ccm() {
		cek_session_akses('investasi',$this->session->id_session);
		$data['record'] = $this->model_investasi->list_penarikan_ccm();
		$this->template->load('administrator/template','administrator/mod_investasi/view_penarikan_ccm',$data);
	}

	function ccm_dikembalikan() {
		cek_session_akses('investasi',$this->session->id_session);
			$data['record'] = $this->model_investasi->list_ccm_dikembalikan();
			$this->template->load('administrator/template','administrator/mod_investasi/view_ccm_dikembalikan',$data);
	...
	function datavoucher(){
		cek_session_akses('datavoucher',$this->session->id_session);
		$data['voucher_data'] = $this->model_voucher->voucher_data();

		$this->template->load('administrator/template','administrator/mod_voucher/view_voucher',$data);
	}
	
	function tambah_kirimvoucher(){
		cek_session_akses('datavoucher',$this->session->id_session);
		if (isset($_POST['submit'])){
			$this->model_voucher->voucher_kirim_tambah();
			redirect('administrator/datavoucher');
		}else{
			$data['tag'] = $this->model_voucher->tag_berita();
			$data['voucher_data'] = $this->model_voucher->voucher_kirim();
			$data['founder_data'] = $this->model_voucher->founder_data();

			$this->template->load('administrator/template','administrator/mod_voucher/view_voucher_tambah',$data);
		}
	}

    function bayar_voucher($kode){
		$data['voucher_bayar'] = $this->model_voucher->voucher_bayar($kode);
		redirect('administrator/datavoucher');
	}
